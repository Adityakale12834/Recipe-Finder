...
function NewRecipe() {
    ...
    async function fetchData() {
        await fetch(`https://api.spoonacular.com/food/search?&apiKey=3201bc5a392343d6ac18d63fe1033452`)
            .then(async (response) => await response.json())
            .then(data => {
                setData(data.searchResults[0].results);
                        ...
                        data.map((item, index) => (
                            <div className=" flex flex-col justify-between border hover:shadow-2xl cursor-pointer object-cover w-full" key={item.id || index} onClick={() => handleNavigate(item.id)}>
                                <div>
                                    ...
                                    <img className='overflow-hidden w-10'  src="/beforefavorite.png" alt="" />
                                </div>
                                <div className='m-2'>
                                    <div>
